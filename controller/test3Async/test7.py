# Test MQTT and Async
# Need to try and find out why it fails
# From test4 adding a wrapper for MQTT so that only sends data rate limited
# Trying no coroutine calls in classes
...
print("test7 version 1.14 2018-08-28")
mq = my_mqtt.MyMqtt()
#mq = MyMqtt()
mq.send_value("0.005", "button")
...
async def mq_queue():
    ...
    while True:
        if len(my_mq.queue) > 0:
            ...
            count = 0
            n += 1
            client = MQTTClient("device_id", "io.adafruit.com",user=mq.user,
                        password=mq.password, port=1883, keepalive=1)
...
async def button_monitor():
    ...
    while True:
        ...
        my_mq.send_value("0.05", "button")
        my_mq.send_value("0.1", "button")
        my_mq.send_value("0.15", "button")
        my_mq.send_value("0.2", "button")
        my_mq.send_value("1", "button")
        while p_in() == 0: # Wait for button release
            await asyncio.sleep_ms(100)
        rgb.set(0x808000)  # Yellow
        my_mq.send_value("0.95", "button")
...
def test(duration):
    ...
    loop.create_task(mq_queue())  # Run first so that any messages are processed
